...
export const setGuessedScore = async (sessionId, gameId, guessedScore) => {
  ...
  if(!Number.isInteger(guessedScore)) {
    throw new Error("Guessed score did not parse to an integer");
  }

  if(guessedScore < ScoreRange.Min || guessedScore > ScoreRange.Max) {
    throw new Error(`Guessed score must be between ${ScoreRange.Min} and ${ScoreRange.Max}. Provided: ${guessedScore}.`);
  }
