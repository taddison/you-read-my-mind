...
const throwIfScoreInvalid = (score) => {
  ...
  if(score < ScoreRange.Min || score > ScoreRange.Max) {
    throw new Error(`Score must be between ${ScoreRange.Min} and ${ScoreRange.Max}. Provided: ${score}.`);
  }
}
...
export const startGame = async (sessionId, gameId, flipRoles) => {
  ...
  if(flipRoles) {
    if(game.sessions.length === 2) {
      ...
      const otherPlayers = game.sessions.filter(s => s.sessionId !== game.psychic && s.sessionId !== game.guesser)
