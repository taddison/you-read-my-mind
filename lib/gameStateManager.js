import {
  ...
  setPsychic,
  setGuesser,
  setPsychicSecrets as setPsychicSecretsDb,
} from "./db";
...
export const setPsychicSecrets = async (sessionId, psychicSubject, leftStatement, rightStatement, psychicScore) => {
  const round = await getRound();
  ...
  if(round.state !== RoundStates.SettingSecrets) {
    throw new Error(`Round is not in the correct state to set secrets - currently ${round.state}`);
  }

  await setPsychicSecretsDb(psychicSubject, leftStatement, rightStatement, psychicScore);
}